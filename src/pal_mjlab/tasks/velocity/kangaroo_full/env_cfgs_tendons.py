...
def pal_kangaroo_full_tendons_cl_rough_env_cfg(
  ...
  cl_simple_model_observable = rf"^(?!leg_.*_length_actuator$|{_cl_exclude_motor}).*$"
...
def pal_kangaroo_full_tendons_flat_env_cfg(
  ...
  cfg = pal_kangaroo_full_tendons_rough_env_cfg(
    play=play, joint_state_obs=joint_state_obs
  )
...
##
# Simple-model-matching hip-tendon variants.
#
# Unlike pal_kangaroo_full_tendons_*_env_cfg above (which drives leg length
# through the "full model" leg_.*_length_actuator/LUT layer and optionally
# observes it), everything below uses the plain leg_.*_length_joint the
# simple kangaroo model itself drives directly, and always observes/rewards
# exactly what pal_kangaroo_flat_env_cfg does -- there is no joint_state_obs
# knob here because there's only one mode.
##


def _set_simple_model_observations(cfg: ManagerBasedRlEnvCfg) -> None:
  """Restrict joint_pos/joint_vel to exactly what pal_kangaroo_flat_env_cfg
  observes: the plain joint set, no tendon lengths. Every variant below has
  the same 26 real joints as the simple kangaroo model (none of them carry
  the LUT-driven leg_.*_length_actuator), so this is a straight match, not
  an approximation.
  """
  for obs in ["actor", "critic"]:
    for term in ["joint_pos", "joint_vel"]:
...
def _set_simple_model_rewards(cfg: ManagerBasedRlEnvCfg) -> None:
  """Same dof_pos_limits/pose reward setup as the tendon-hips variant uses,
  which is already scoped to the simple model's joint/actuator names.
  """
  cfg.rewards["dof_pos_limits"] = RewardTermCfg(
    func=mdp.joint_pos_limits,
    weight=-1.0,
...
def pal_kangaroo_full_tendons_simple_knee_rough_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  """Create PAL Robotics KANGAROO FULL rough terrain velocity configuration.

  Both hip_z and hip_xy are tendon actuated (as in the tendon-hips variant),
  but leg length is the "simple model" leg_.*_length_joint driven directly --
  no leg_.*_length_actuator/LUT layer, no knee_rods tendon -- so observations
  and rewards match pal_kangaroo_flat_env_cfg exactly.
  """
  cfg = pal_kangaroo_baseline_env_cfg(play)

  cfg.scene.entities = {"robot": get_kangaroo_full_robot_tendon_simple_knee_cfg()}

  cfg.actions = {
    "joint_pos": JointPositionActionCfg(
      entity_name="robot",
      actuator_names=KANGAROO_FULL_TENDON_SIMPLE_KNEE_ACTUATED_JOINTS_NAMES,
      scale=KANGAROO_FULL_TENDON_SIMPLE_KNEE_JOINT_ACTION_SCALE,
      use_default_offset=True,
    ),
    "tendon_pos": TendonLengthActionCfg(
      entity_name="robot",
      actuator_names=(r"left_hip_z_slider$", r"right_hip_z_slider$"),
      preserve_order=True,
      scale=KANGAROO_FULL_TENDON_SIMPLE_KNEE_HIPZ_TENDON_ACTION_SCALE,
      offset=KANGAROO_INIT_STATE_SIMPLE_KNEE_HIPZ_TENDONS_OFFSETS,
    ),
    "hip_xy_pos": TendonLengthActionCfg(
      entity_name="robot",
      actuator_names=(
      ...
      preserve_order=True,
      scale=KANGAROO_FULL_TENDON_SIMPLE_KNEE_HIPXY_TENDON_ACTION_SCALE,
      offset=KANGAROO_INIT_STATE_SIMPLE_KNEE_HIPXY_TENDONS_OFFSETS,
    ),
  }

  _set_simple_model_observations(cfg)
  _set_simple_model_rewards(cfg)

  return cfg


def pal_kangaroo_full_tendons_simple_knee_flat_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  """Create PAL Robotics KANGAROO FULL (simple knee) flat terrain velocity configuration."""
  cfg = pal_kangaroo_full_tendons_simple_knee_rough_env_cfg(play=play)
...
def pal_kangaroo_full_tendons_hip_z_only_rough_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  """Create PAL Robotics KANGAROO FULL rough terrain velocity configuration.

  Only hip_z is tendon actuated. hip_xy (leg_.*_2_joint pitch + leg_.*_3_joint
  roll) and leg length (leg_.*_length_joint) stay real joints driven directly,
  exactly as in the simple kangaroo model.
  """
  cfg = pal_kangaroo_baseline_env_cfg(play)

  cfg.scene.entities = {"robot": get_kangaroo_full_robot_tendon_hip_z_only_cfg()}

  cfg.actions = {
    "joint_pos": JointPositionActionCfg(
      entity_name="robot",
      actuator_names=KANGAROO_FULL_TENDON_HIP_Z_ONLY_ACTUATED_JOINTS_NAMES,
      scale=KANGAROO_FULL_TENDON_HIP_Z_ONLY_JOINT_ACTION_SCALE,
      use_default_offset=True,
    ),
    "tendon_pos": TendonLengthActionCfg(
      entity_name="robot",
      actuator_names=(r"left_hip_z_slider$", r"right_hip_z_slider$"),
      preserve_order=True,
      scale=KANGAROO_FULL_TENDON_HIP_Z_ONLY_TENDON_ACTION_SCALE,
      offset=KANGAROO_INIT_STATE_HIP_Z_ONLY_TENDONS_OFFSETS,
    ),
  }
...
def pal_kangaroo_full_tendons_hip_z_only_flat_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  """Create PAL Robotics KANGAROO FULL (hip_z tendon only) flat terrain velocity configuration."""
  cfg = pal_kangaroo_full_tendons_hip_z_only_rough_env_cfg(play=play)
...
def pal_kangaroo_full_tendons_hip_xy_only_rough_env_cfg(
  ...
  cfg.scene.entities = {"robot": get_kangaroo_full_robot_tendon_hip_xy_only_cfg()}

  cfg.actions = {
    "joint_pos": JointPositionActionCfg(
      entity_name="robot",
      actuator_names=KANGAROO_FULL_TENDON_HIP_XY_ONLY_ACTUATED_JOINTS_NAMES,
      scale=KANGAROO_FULL_TENDON_HIP_XY_ONLY_JOINT_ACTION_SCALE,
      use_default_offset=True,
    ),
    "hip_xy_pos": TendonLengthActionCfg(
      entity_name="robot",
